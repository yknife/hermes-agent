...
def _eager_reconcile_own_session_db() -> None:
    ...
    try:
        ...
        _log.warning(
            ...
            "reads will retry the heal per poll",
            exc,
        )


async def _start_desktop_api_server():
    """Start Hermes' authenticated OpenAI-compatible listener for Desktop.

    Desktop embeds its chat gateway in this dashboard process instead of
    running ``hermes gateway run``.  The API-server platform therefore does
    not get a normal platform lifecycle, even though Electron intentionally
    supplies a strong process-local API_SERVER_KEY.  Video Knowledge uses this
    Hermes-owned loopback surface for analysis, so own the adapter alongside
    the dashboard and close it from the same lifespan.
    """
    if os.getenv("HERMES_DESKTOP") != "1":
        return None
    enabled = os.getenv("API_SERVER_ENABLED", "").strip().lower()
    if enabled not in {"1", "true", "yes", "on"}:
        return None

    api_key = os.getenv("API_SERVER_KEY", "")
    if not api_key:
        _log.error("Desktop API server not started: API_SERVER_KEY is missing")
        return None
    ...
    raw_port = os.getenv("API_SERVER_PORT", "").strip()
    if raw_port:
        extra["port"] = raw_port
    model_name = os.getenv("API_SERVER_MODEL_NAME", "").strip()
    if model_name:
        extra["model_name"] = model_name

    adapter = APIServerAdapter(PlatformConfig(enabled=True, extra=extra))
    if not await adapter.connect():
        _log.error("Desktop API server failed to start")
        return None
    return adapter
...
async def _lifespan(app: "FastAPI"):
    ...
    if os.getenv("HERMES_DESKTOP") == "1":
        ...
        desktop_api_server = await _start_desktop_api_server()
    ...
    try:
        ...
        if desktop_api_server is not None:
            await desktop_api_server.disconnect()
        if os.getenv("HERMES_DESKTOP") == "1":
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    def _run():
        ...
        with _profile_scope(profile) as scoped_home:
            ...
            stem = (
                Path(_sanitize_chat_image_filename(payload.filename)).stem
                or "pasted-image"
            )
    ...
    try:
        with _config_profile_scope(profile):
            ...
            if not model or not base_url:
                raise HTTPException(
                    status_code=400, detail="custom endpoint is incomplete"
                )
        ...
        try:
            with cron_jobs.use_cron_store(home):
                ...
                if not isinstance(provider, InProcessCronScheduler):
                    profile_names = [
                        str(p.get("name") or "") for p in _cron_profile_dicts()
                    ]
    ...
    if profiles_root.is_dir():
        ...
        for entry in entries:
            ...
            if not entry.is_dir() or not profiles_mod._PROFILE_ID_RE.match(entry.name):
                continue
            model, provider = _safe(
                lambda entry=entry_path: profiles_mod._read_config_model(entry),
                (None, None),
            )
            profiles.append({
                "name": entry.name,
                ...
                "provider": provider,
                "has_env": _safe(
                    lambda entry=entry_path: (entry / ".env").exists(), False
                ),
                "skill_count": _safe(
                    lambda entry=entry_path: profiles_mod._count_skills(entry), 0
                ),
                "gateway_running": _safe(
                    lambda entry=entry_path: profiles_mod._check_gateway_running(entry),
                    False,
                ),
                "description": _safe(
                    lambda entry=entry_path: profiles_mod.read_profile_meta(entry).get(
                        "description", ""
                    ),
                    "",
                ),

...
try:
    from plugins.video_knowledge.backend.app.domain.errors import (
        DomainError as _VideoKnowledgeDomainError,
    )
    ...
    from plugins.video_knowledge.backend.app.integration.runtime import (
        runtime_registry as _video_knowledge_registry,
    )
    from plugins.video_knowledge.backend.media_adapters.errors import (
        MediaToolError as _VideoKnowledgeMediaToolError,
    )

    VIDEO_KNOWLEDGE_AVAILABLE = True
except ImportError:
    _VideoKnowledgeDomainError = None  # type: ignore[assignment,misc]
    _VideoKnowledgeMediaToolError = None  # type: ignore[assignment,misc]
    VideoKnowledgeController = None  # type: ignore[assignment,misc]
    _video_knowledge_registry = None
    VIDEO_KNOWLEDGE_AVAILABLE = False
...
def _request_structured_mode(model_options: Any) -> bool:
    return isinstance(model_options, dict) and _coerce_request_bool(
        model_options.get("structured_mode"), default=False
    )
...
def _request_max_tokens(model_options: Any) -> Optional[int]:
    if not isinstance(model_options, dict):
        return None
    value = model_options.get("max_tokens")
    if isinstance(value, bool) or not isinstance(value, int):
        return None
    return min(max(value, 256), 16_384)


def _structured_output_instruction(body: Any) -> Optional[str]:
    """Translate OpenAI response_format into an agent-visible constraint.

    The API server runs a Hermes agent rather than forwarding the incoming
    request verbatim, so provider-level ``response_format`` would otherwise be
    discarded. Preserve the OpenAI contract by making the requested schema an
    ephemeral instruction for this turn. The downstream provider can still
    enforce JSON mode when supported by its normal Hermes runtime.
    """
    ...
    if format_type == "json_object":
        return (
            "Return exactly one valid JSON object. Do not include reasoning, "
    ...
    return (
        "Return exactly one valid JSON object matching the following JSON "
        "Schema. Do not add properties excluded by the schema, and do not "
        "include reasoning, prose, or Markdown fences.\n\nJSON Schema:\n" + schema_text
    )
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._video_knowledge_registry = _video_knowledge_registry
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        if VIDEO_KNOWLEDGE_AVAILABLE:
            routes.extend([
                ("*", "/api/video-knowledge/v1", self._handle_video_knowledge),
                (
                    "*",
                    "/api/video-knowledge/v1/{tail:.*}",
                    self._handle_video_knowledge,
                ),
            ])
        if _CRON_AVAILABLE:
    ...
    async def _handle_video_knowledge(self, request: "web.Request") -> "web.Response":
        """Serve the profile-scoped Video Knowledge API through this gateway."""
        auth_error = self._check_auth(request)
        if auth_error is not None:
            return auth_error
        if self._video_knowledge_registry is None or VideoKnowledgeController is None:
            return web.json_response(
                {
                    "error": {
                        "code": "NOT_AVAILABLE",
                        "message": "Video Knowledge unavailable",
                    }
                },
                status=503,
            )
        ...
        gateway_url = f"http://127.0.0.1:{self._port}{profile_prefix}"
        from hermes_constants import get_hermes_home

        runtime = await self._video_knowledge_registry.get(
            get_hermes_home(),
            gateway_base_url=gateway_url,
            gateway_api_key=self._expected_api_key() or None,
        )
        ...
        try:
            result = await VideoKnowledgeController(runtime).dispatch(
                ...
                payload=payload,
                actor=f"hermes-api:{profile or 'default'}",
            )
        except Exception as exc:
            ...
            if _VideoKnowledgeMediaToolError is not None and isinstance(
                exc, _VideoKnowledgeMediaToolError
            ):
                return web.json_response(
                    {
                        "error": {
            ...
            if exc.__class__.__module__.startswith("pydantic") or isinstance(
                exc, ValueError
            ):
                return web.json_response(
            ...
            logger.exception("Video Knowledge request failed")
            return web.json_response(
    ...
    def _create_agent(
        ...
        structured_response_format: Optional[Dict[str, Any]] = None,
        route: Optional[Dict[str, Any]] = None,
        ...
        structured_mode = _request_structured_mode(model_options)
        ...
        agent_kwargs = {
            ...
            **_checkpoint_agent_kwargs(user_config),
            ...
            "skip_context_files": structured_mode,
        }
        if structured_mode and isinstance(structured_response_format, dict):
            # The gateway normally turns response_format into an agent-visible
            # instruction. Structured batch calls also need the original
        ...
        # Structured batch callers need a bounded, independently retryable
        # response. General chat length continuation can otherwise multiply
        # the caller's explicit output cap up to 32K.
        agent._disable_length_continuation = structured_mode
    ...
    async def _handle_patch_session(self, request: "web.Request") -> "web.Response":
        ...
        for flag in ("pinned", "archived", "hidden", "unread"):
            if flag in body and not isinstance(body[flag], bool):
                return web.json_response(
                    _openai_error(
                        f"'{flag}' must be a boolean", code="invalid_session_field"
                    ),
                    status=400,
                )
        ...
        if "archived" in body:
            await asyncio.to_thread(
                db.set_session_archived, session_id, body["archived"]
            )
        if "hidden" in body:
            await asyncio.to_thread(db.set_session_hidden, session_id, body["hidden"])
        if "unread" in body:
            await asyncio.to_thread(
                db.set_session_read, session_id, read=not body["unread"]
            )
    ...
    async def _handle_session_chat_stream(
        ...
        try:
            ...
            logger.info(
                "Session SSE client disconnected; interrupted live run %s", run_id
            )
        except asyncio.CancelledError:
            await self._drain_session_stream_task_on_disconnect(
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        structured_instruction = _structured_output_instruction(body)
    ...
    async def _handle_steer_run(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            return web.json_response(
                _openai_error(_redact_api_error_text(exc), code="steer_failed"),
                status=500,
            )
        if not accepted:
            return web.json_response(
                _openai_error(
                    f"Run did not accept steer text: {run_id}",
                    code="steer_not_accepted",
                ),
                status=409,
            )
    ...
    async def disconnect(self) -> None:
        ...
        if self._video_knowledge_registry is not None:
            await self._video_knowledge_registry.stop_all()

...
class TestNonStreamingAgentCancelOnDisconnect:
    """Non-streaming clients must abort the live upstream model request."""
    ...
    async def test_normal_completion_is_not_interrupted(self):
        ...
        async def compute(agent_ref, disconnect_event):
            assert agent_ref == [None]
            assert not disconnect_event.is_set()
            return {"final_response": "done"}, {"total_tokens": 1}

        result = await adapter._await_nonstreaming_completion(request, compute)

        assert result[0]["final_response"] == "done"

    @pytest.mark.asyncio
    async def test_disconnect_hard_interrupts_agent(self):
        adapter = _make_adapter()
        request = _make_request()
        request.transport = MagicMock()
        request.transport.is_closing.side_effect = [False, True]
        ...
        async def compute(agent_ref, disconnect_event):
            ...
            return {"final_response": "interrupted"}, {"total_tokens": 0}

        with pytest.raises(ConnectionResetError):
            await adapter._await_nonstreaming_completion(request, compute)

        assert agent.message == "Non-streaming API client disconnected"
